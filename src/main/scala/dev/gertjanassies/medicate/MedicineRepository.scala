...
class MedicineRepository(redis: Redis) {
  ...
  def getAll: Task[List[Medicine]] = for {
    ...
    medicines <- ZIO.succeed(values.flatten.flatten.flatMap(_.fromJson[Medicine].toOption))
  } yield medicines.toList

  def getById(id: String): Task[Option[Medicine]] =
    redis.get(s"$prefix$id").returning[String].map(_.flatMap(_.fromJson[Medicine].toOption))
